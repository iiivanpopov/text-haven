...
export class Cache {
	constructor(private redis: RedisClient, private prisma: PrismaClient) {}

	static mapKey(type: CacheEntityType, args: CacheKeyParams = {}): string | null {
	...
	async set(key: string, value: unknown, expiration = config.CACHE_EXPIRE_TIME): Promise<void> {
