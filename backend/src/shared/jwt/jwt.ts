...
export class Jwt {
	...
	validateToken(token: string, tokenType: TokenType): UserDto | null {
		try {
			const secret = tokenType == TokenType.ACCESS ? this.accessSecret : this.refreshSecret
...
export default new Jwt(config.PRISMA, config.JWT_SECRET_KEY, config.REFRESH_SECRET_KEY)
