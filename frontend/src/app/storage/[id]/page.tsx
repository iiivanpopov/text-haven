...
export default function Storage({ params }: { params: Promise<{ id: string }> }) {
	...
	const filteredFiles = useMemo(() => files.filter(file => regex.test(file.name)), [files, regex])

	return (
		<main className='mt-20 w-3/5 pb-40'>
			<h3 className='mb-5 text-5xl flex flex-col text-gray-800 dark:text-gray-100'>Storage</h3>
			<Input
				name='Search'
				value={search}
			...
			{filteredFolders.map(folder => (
				<FolderBtn key={folder.id} folderId={folder.id} className='px-5' title={folder.name} />
			))}

			{filteredFiles.map(file => (
				<FileBtn key={file.id} className='px-5' fileId={file.id} title={file.name} />
			))}
